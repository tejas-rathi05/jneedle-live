  ...
  const pageQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${conf.baseURL}/api/pages/fetch-page-item`, {
        ...
        body: JSON.stringify({
          pageHref: params.collectionName,
        }),
      });
      const data = res.json();
      return data;
  ...
  return (
    <section>
      {pageQuery.isSuccess &&
        ...
        pageQuery.data.pageHeading !== "" && (
          <h2 className="text-3xl md:text-4xl font-light tracking-wider text-center my-10 uppercase">
            {pageQuery.data.pageHeading}
          </h2>
        )}
      {pageQuery.isSuccess &&
        pageQuery.data &&
        pageQuery.data.billboard.previewUrl && (
          <div>
            <img src={pageQuery.data.billboard.previewUrl} alt="" />
          </div>
        )}
